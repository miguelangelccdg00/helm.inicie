...
class StoreSolucionesController 
{
     ...
     * @returns {Promise<void>} Devuelve el resultado de la actualización de los ámbitos de la solución.
     */
    async updateSolucionAmbitosSectores(req: Request<{ id: string }, any, any[]>, res: Response<any>): Promise<void> 
    {
        try 
        {
            const { id } = req.params;
            const solucionAmbitosSectores: any[] = req.body;
            ...
            if (!Array.isArray(solucionAmbitosSectores) || solucionAmbitosSectores.length === 0) 
            {
                res.status(400).json({ message: 'Se requiere un array de ámbitos o de sectores para actualizar' });
                return;
            }
            ...
            const result = await StoreSolucionesService.updateSolucionAmbitosSector(Number(id), solucionAmbitosSectores);
            res.json(result);
        } 
        catch (error) 
        {
